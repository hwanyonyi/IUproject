...
spark = SparkSession \
    .builder \
    .appName("Weekday processing") \
    .config("spark.sql.warehouse.dir", warehouse_location) \
    .enableHiveSupport() \
    .getOrCreate()

# Read the file weekday_data.json from the HDFS
df = spark.read.json('hdfs://namenode:9000/weekday/weekday_data.json')
...
weekday_data = df.select('datetime', 'street_id', 'count', 'vel') \
...
weekday_data.write.mode("append").insertInto("weekday_traffic")
